...
namespace ELKStackApp.Extensions;

public static class SerilogExtensions
{
    static void LogEventCallback()
    {
    }
    ...
    {
#if DEBUG
        StreamWriter selfLogFile = new("./logs/serilog-selflog.txt", append: true)
        {
            AutoFlush = true
        };
                ...
                .MinimumLevel.Information()
                    ...
                    .Elasticsearch(
                        new ElasticsearchSinkOptions(
                            ...
                            {
                                IndexFormat =
                                    $"{$"{Assembly.GetExecutingAssembly().GetName().Name!.ToLower().Replace("." , "-")}"}{$"-{context.HostingEnvironment.EnvironmentName?.ToLower().Replace("." , "-")}-{TimeProvider.System.GetUtcNow():yyyy-MM}"}",
                            ...
                            Console.WriteLine($"Unable submit event to ElasticSearch{e.InnerException}"),
                                ...
                                BufferLogShippingInterval = TimeSpan.FromSeconds(2),
                                ...
                                ModifyConnectionSettings =
                                    conn =>
                                    {
                                        ...
                                        conn.EnableDebugMode(_ => LogEventCallback());
#endif
                                        return conn;
                                    }
                            })
                    ...
                    .WriteTo
                    .Console(theme: SystemConsoleTheme.Colored, applyThemeToRedirectedOutput: true);
            });
        return hostBuilder;
    }
}